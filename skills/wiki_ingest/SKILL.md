...
3.  **File Type Handling & Conversion**: 
    *   **For `.pdf` files**: You **MUST** enforce strict pagination to prevent laziness and truncation. You **MUST** use the `invoke_subagent` tool to spawn parallel subagents, assigning each subagent exactly ONE page of the PDF to transcribe using their native multimodal vision. 
