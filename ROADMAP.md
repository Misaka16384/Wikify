...
>
> 最后更新：2026-08-23 · 当前阶段：**v1.12.4 已发布**，之后有未发布改动（A12/A13、B1、rung 5 表格与切图、rung 2 两类老方言）。**阶梯五级的质量现在全部有实测数字**（日·五），此前：M0–M9 全部完成。
>
...
- ~~本地 OCR（rung 5）在数学上的质量从未测过~~ **已测，2026-08-23，见当天条目**：公式好（干净页 39 行里 3 行真错），~~表格全丢（A9）~~**表格是提示词丢的，不是模型丢的，已修（见 8-23 日·续）**，会复读（A10，仍在）。~~**仍没测的是扫描件**~~ **已测，2026-08-23（日·五）**：扫描件上公式质量**不降**（5/5 exact，倾斜+噪点+JPEG60 的劣化版同样 5/5）；但表格塌到 10/49，因为切图判据读文本层而扫描件没有，**强制 2×2 切图恢复到 49/49**
...
- ~~**切图对扫描件有没有用，没测。**~~ **已测（日·五）：有用，而且是数量级的**——同一张扫描的表格页，
  整页 24/49（20.4%，且复读），2 竖切 91.8%，**2×2 达到 100%，与原生 PDF 打平**。
  判据读文本层而扫描件没有，所以现在拿不到。**这和当天早些时候修的数学门是同一个形状："没看成"被当成了"没有"**。
  优先级不高的唯一理由是：扫描件先走 rung 4，而 rung 4 在扫描件表格上是 49/49（见下），
  所以这条只影响纯离线路径
