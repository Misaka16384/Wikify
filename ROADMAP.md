...
>
> 最后更新：2026-08-24 · 当前阶段：**v1.12.5 已发布**，之后清掉三条挂账，再把全盘 review 的 35 条 + D 栏按失败形状修完（P0–P8）。**阶梯五级的质量现在全部有实测数字**（8-23 日·五），此前：M0–M9 全部完成。测试 **1294**，8 个 commit 未 push。
>
> 2026-08-24（一·二）全盘 review 的 35 条 + D 栏，按**失败形状**重排后逐段修完。用户「全面按顺序修复」。**35 条是 35 个文件，但只有 8 种病。**
>
> 排序不按评审的编号，因为按文件修等于把同一种病修三遍、并且漏掉第四处——⑥（图标题里的 `[36]`）和 ⑧（`MarkdownCleaner` 劈表格行）当初就是这么漏的：两处都是"最后一公里的正则手术"，但分属两个文件，所以谁也没把它们当成同一件事。
>
> **P0 一篇少了五页的论文，在审阅队列里显示为"成功、零错误、零 flag"。** `agent.py` 无条件 `success=True`，`batch.py` 的 error 字段又以 success 把关，失败页也不产生任何 finding。而**昨天刚发的 `review_order` 正是按 `loud_findings` 排序的**——这条 bug 把信号清零，排序在最该起作用的那类文档上失效，队列反而把它排到最后。
>
> 修法是 finding 不是布尔值：评审建议 `success=False` 且清空 `markdown_path`，那会把 40 页里好的 39 页一起扔掉。守卫不是 grep，是让每条路线都必须经过的那道缝自己发现——`ConversionResult.silent_about` 定义"没人会看见的错误"，`batch` 在缝上把它转成 finding。
>
> **P1 三处正则在所有门后面改写 markdown，而两处漏掉的正好是对方有的。** "哪些跨度不能碰"的判断仓库里各写了一份：OCR 那份认表格不认代码块，KB 那份认代码块不认表格。抽成 `core/md_blocks` 共用。`format_math` 的 `$$A$$ and $$B$$` 要**两处一起改才挡得住**（只加 `count=='2'` 前置正则先炸，只改正则逐行分支接着炸），规则是**一行里多于一对 `$$` 就整行不碰**——任何改写都得决定数学在哪结束、散文从哪开始，而这里没人知道。`format_math` 此前零测试覆盖，先钉行为再动手，改完 15 个用例逐字节比对：只有那两条坏的变了。
>
> **P2 D4 不是 JS 重实现 Python，是两份 Python 实现、其中一份是错的。** 而且**就在为了 D4 才共用 `review_order` 的那个功能里面**。这条要连后果一起说清楚：WebUI 硬编码 `source_type="arxiv"` **当时没炸**（route 是显式传的），写进账本的是一句假话——**用错误的后果论证的正确改动，会被下一个真去查的人退回去**。
>
> **P3 同一个形状 v1.12.2 修过一次，两个版本内在六个地方回来了。** `magi ingest auto --topic-dir B` 从 A 执行，读 A 的配置写 B 的库。那次是点修没有守卫，所以这次**守卫是交付物、六处修复是它的结果**：声明了 `--topic-dir` 的模块不许出现无参数的 `load_config()`。这条扫描当场抓出第七处。
>
> **P4 十处资源在失败路径上没被释放，问法都是同一句**："下一行抛异常的时候，这个东西怎么办？" 其中 `jobs.py` 那个取消窗口仍在调 `proc.terminate()`，而 `cancel_job` 早就改用 `_terminate_tree` 了——外壳死了、pandoc 没死。
>
> **P5 一个截断的 `registry.json` 会静默变成"没有 `registry.json`"。** 半个文件解析成"一个库都没有"，下一次保存把这个空写回去盖掉原件，什么都不打印。**只换原子写不够**（那只挡半个文件），锁必须罩住整段 load-modify-save 才挡丢更新——而 `magi index` 每次跑完都注册一次工作区，**两个 index 同时结束是常态**。
>
> **P6 那个写着 "possible ReDoS attack" 的守卫什么都没防住。** 五秒超时包的是 `re.compile`（很快），指数回溯在 `pattern.search`（无保护）。**一个自报家门却不存在的防御比没有防御更糟**，它替下一个读代码的人把问题回答掉了。评审建议的线程池也不成立（跑着的 `re` 持 GIL）。换成诚实的墙钟预算 + 明说"挡不住单行的灾难性匹配，标准库里没有东西能挡"。
>
> **P7 性能十条，评审自己的二次核验把全部倍数都撤了**（9.5x / 140x / 50.65s→0.36s 在仓库里没有可复现材料）。只做四条不看倍数也对的，**明确不做 R28**（拿 mtime 当内容未变的充分条件会**降低正确性**）**和 R30**。顺带修正评审示例两处：`bisect_left` 不是 `bisect_right`（`str.count(x,0,pos)` 不含 pos 本身），复杂度是 `O(N + M log K)` 不是 `O(M log K)`。
>
> **P8 三条架构债落地，两条明说没还完。** D4 摄取侧收敛（JS 侧仍在）、D3 写成 `core/durability.py`（`unknown` 是真实答案不是失败——猜是怎么把 `output/ingest/` 塞进"删掉 output/ 重来"的）、D6 写成 `docs/degradation.md` 八行 + 8 条测试、D5 最小版（12 个 skill 补 `## Rules`，**但行为仍然测不了**）、**D9 新增**。
>
> 测试 1118 → **1294**（+176）。8 个 commit，**没有 push**。
>
> 2026-08-24（一）三条"还没做的"做完，其中两条的结论和记录里写的相反。用户「还没做的做完」。
>
> **① 扫描件切图判据：这条记录本身是错的，而且错在评分器上——今天是第三次。**
>
> 原记录说"同一张扫描的表格页整页 24/49（20.4%），2×2 达到 100%"，病因归给"判据读文本层而扫描件没有"。用 `magi ingest ocr` **自己的渲染器**重测同一份 PDF：
>
> | 49 行表，两格全对 | 整页 | 左右两块 | 2×2 |
> |---|---|---|---|
> | 原生 PDF | 24/49 (49.0%) | **49/49** | 49/49 |
> | 干净扫描件 | 24/49 (49.0%) | **49/49** | 47/49 |
> | 劣化扫描件（倾斜+噪点+JPEG60） | 24/49 (49.0%) | 45/49 (91.8%) | 49/49 |
>
> **三种来源整页都是 24 行，一个不差。** 所以这从来不是扫描件的问题，是模型的定额——和 A10 记的 EOS 是同一件事，只是当时没意识到它跟来源无关。
>
> 20.4% 那个数是评分器打出来的：它要求 `[[n,k,d]]` **双**方括号，而 glm-ocr 写的是 `[n,k,d]`（经典码的记法，同样三个数字）。**四份完整的转写因为括号个数全判零分。** 判据还是老那条，而且这次是它自己报的警：**干净扫描件 0%、更脏的扫描件 91.8%——这种不合常理的差距先查评分器，不要先查模型。**
>
> **② 修法不是"给扫描件补一个图像版判据"。** 那条先试了并且实测否掉（进了 C 栏）：靠横线检测在**原生**页上就分不开（29 个真表格页里 18 个没有跨 30% 宽度的横线，44 个非表格页却有），而且**它恰恰在该起作用的地方崩掉**——同一张扫描页，干净版检出 27 条横线，倾斜 0.4° 后检出 **0** 条。**一个只在不需要它的时候才准的判据。**
>
> 真正的修法是**问转写本身**：整页读回来如果带着一张长表，就换一种切法重读一次，由同一道门判优劣。这把 D8 从"复读才修"扩成"两种症状都修"，顺带覆盖了原生 PDF 上 `find_tables` 漏掉的表——判据不再依赖源文件能不能被读。
>
> **③ 切法必须跟着页面内容走，这是把矩阵测全才看出来的。** 端到端跑劣化扫描件，2×2 修完回来 **100 个 pipe 行，而表只有 49 行**——横切正好落在表格中间，每半行被当成独立一行，**出来是一张读着很整齐、而且是错的表**。两竖切在三种来源上是 49/49/45。但两竖切**也正是**把某个散文页从 559 字符变成 32,785 字符复读的那个切法。所以规则是：**有表格就左右切，没表格就 2×2**——症状决定要不要修，页面内容决定怎么修，这是两个问题。
>
> **④ MinerU 的图查了，不用动（E 栏关掉一条）。** 8 张图的对照件上 MinerU 出 11 个文件，看着像碎片化，实际是**把多面板图拆成各个面板、每个面板配自己的子标题，而主标题 `FIG. 3.` 也留着**。8 张一张不少，粒度比我们的整图裁剪更细。**"看起来数目对不上"不等于"丢了东西"。**
>
> **⑤ rung 3 接上图片裁剪。** `figure_extractor` 从 `ocr/` 挪到 `magi/ingest/figures.py`——它从来不碰 OCR，只读 PDF 自己的绘制框和文本层。文本层路线现在**按页**注入裁剪图：按页是必须的，整篇搜 "Figure 2" 会先撞上正文里提到 Figure 2 的那句话，图就落到几段之外去了。两篇真论文各 8/8，全部落在自己的标题上方。`ingest.textlayer_images` 从"开关"变成"二选一"（裁剪 vs 导出全部嵌入对象），后者对没有标题的文档仍然是唯一能出东西的路。
>
> **⑥ 顺手撞出一个一直在发的 bug，形状正是 `image_refs` 那篇注释最怕的那种。** 图片的 alt 文本就是图标题，而物理标题会引用：`FIG. 1. ... Pauli operators [36].`。那个方括号**提前关掉了 alt 文本**，整行不再是图片引用——渲染器显示半截标题加一段字面量 `](images/x.png)`。更糟的是 **`image_refs` 也解析不到它，而所有图片检查都是先找引用**，所以一张坏掉的图会被报成"这篇没有图，干净"。8 张里中招 1 张，**而它一直在发**。写的一侧转义，读的一侧认转义，两边都补了测试。
>
> **⑦ 审阅排序：先测再做，结论是"可以排，但不能据此免检"。** 16 份真实转换（MinerU / 本地 OCR，原生件和扫描件，质量独立已知）过一遍真实的门：**4 份带 flag，其中包含全部 3 份已知坏的**。所以 flag 有选择性（25%），指向也对——排序是有意义的。
>
> **但同一批数据也否掉了下一步。** 里面有一份**丢了一半表格、却一个 flag 都没有**。所以"没 flag"不等于"查过了"，这只能是排序，不能变成"其余批准"。CLI 和 WebUI 共用同一个 `ledger.review_order`（D4 那条债的反面教材就在旁边）。
>
> **⑧ 端到端复核时又抓出一个，而且它在所有门的后面。** 引擎吐出 53 个 pipe 行，写进文件变成 **72** 个。查出来是 `MarkdownCleaner._fix_latex`：它给行间公式前后补空行，而模型在单元格里留了个多余的 `$$`——于是 **19 行表格被劈成两行**。`$$` 到处都是行间公式的定界符，唯独在单元格里不可能是（单元格装不下一个块）。
>
> **这一步跑在模型、表格转换、门、修复**全部之后**，所以它做什么都没有下游会发现**。出来又是那个形状：不崩、不缺、一张读着很整齐而且带着文档里不存在的行的表。改成按块处理、表格行原样放过：**63.3% → 91.8%**，只改这一处。
>
> **⑨ 顺带：`/api/chat` 从来没设过 temperature。** 它是**默认**路径（任何没有单独配置的模型都走它），所以 `qwen3-vl` 那一族一直在 Ollama 的默认 **0.8** 上做转写。**`glm-ocr` 不受影响**（它 `use_chat_api: False`，走 `/api/generate`，那边一直设着 0）——所以今天所有数字都跟这条无关，但换模型的人会撞上。**转写要的是最可能的那个 token，不是多样性**，"编一个看着合理的数"正是本管线最怕的失败。
>
> **⑩ 确定性的边界，量清楚了（因为修复机制的前提就是它）。** 同一份预处理后的字节、同一个进程里连读三次：**5967 字符、sha256 完全相同，三次一模一样**。所以"不改参数的重试毫无意义"是对的，D8 的前提站得住。
>
> **但跨进程、隔一段时间跑，同一份 PDF 的整页读数不一样**（这一轮里见到 4,893 / 5,472 / 5,967 / 42,183 / 44,960 字符，有的复读有的不复读）。**所以笔记里的具体字符数不要当成可复现的常量**——它们是那一次的观测。可复现的是**方向**：整页 24/49，切开 45–49/49，三种来源都一样。
>
> **端到端确认（`magi ingest ocr` 本身，不是脚本）：**
>
> | 扫描的表格页 | 今天之前 | 现在 |
> |---|---|---|
> | 干净版 | 63.3%，72 个 pipe 行 | **100.0%**，53 行（整页复读 → 左右两块） |
> | 劣化版 | **49.0%，而且一个 flag 都没有** | **91.8%**，53 行（新判据点名命中：*came back with a table long enough to have been cut short*） |
>
> 测试 1082 → **1118**。
>
> 2026-08-23（日·五）一轮补测：MinerU、扫描件、按页路由。用户「决策相关没测的都测」。**最重要的一条是我的评分器测错了一次，而且差点写进结论。**
>
> **① 先说测错的那次。** 第一遍打出来 MinerU 公式 exact **16.9%**、glm-ocr **66.7%**——四倍差距。但 `wrong` 两边几乎一样（9.2% vs 10.4%），**这不自洽**：读错了才进 wrong，差距全堆在 exact→minor 之间，那是记法差异不是识别差异。查出来：
>
> ```
> MinerU  U_{x}            真值  U_x
> MinerU  \varepsilon_{j}  真值  \varepsilon_j
> MinerU  H_{{hop}}        真值  H_{hop}
> ```
>
> 数学上完全一样。`grade2.norm` 只折叠**单字符**下标 `_{x}→_x`，而 MinerU 给每个下标都加花括号；`\mathrm` 剥掉后还留下双层花括号。**评分器在量自己的书写习惯。** 归一化器加了"折叠任意长度的花括号下标 + 折叠双花括号"，**所有变体一起重打**——只修一边的数字比不修更糟。
>
> **判据记下来：一个差距大到不合常理、而错误率却不动的对比，先怀疑评分器。**
>
> **② MinerU vs 本地，同九页、同评分器、绝对数：**
>
> | | 公式行 | exact | minor | degraded | wrong |
> |---|---|---|---|---|---|
> | **rung 4 MinerU** | 65 | **42** | 7 | 11 | **5** |
> | rung 5 glm-ocr（新提示词） | 48 | 36 | 2 | 5 | **5** |
> | rung 5 glm-ocr（旧提示词） | 45 | 34 | 3 | 5 | 3 |
> | DeepSeek 2×2 | 65 | 42 | 4 | 12 | 7 |
>
> **错的行数一样（5 vs 5），MinerU 多捞回 6 个完全正确的公式。** 按百分比 glm-ocr 更高（75.0% vs 64.6%），但那只是因为它吐得少。表格更是一边倒：**toric p4 的 49 行表，MinerU 在原生 PDF、干净扫描件、劣化扫描件上全部 49/49，而且不需要切图**；glm-ocr 整页 24/49，要 2×2 才 100%。速度上 MinerU 也快——3 页端到端 15.6 秒（含上传、转换、下载），约 5s/页且云端可并发，本地是 12–30s/页串行。
>
> **所以阶梯顺序是对的，我的假设错了。** 我怀疑 rung 4 该降到 rung 5 下面，实测正好相反。rung 5 是正确的**离线兜底**，不是更好的选择。
>
> **顺带修正当天早些时候的一句话**：我说"DeepSeek 不需要了，因为 glm-ocr 追平了它"——用修好的评分器看，**DeepSeek 切图其实强过 glm-ocr**（42 vs 36 exact）。不该加它的真正理由是 **MinerU 全面压住它**：同样 42 exact 但只错 5 个（它 7 个），更快，表格不用切图，而且已经在阶梯里。**结论没变，理由是错的，而错的理由会在下次决策时误导人。**
>
> **③ 扫描件：rung 5 的存在理由第一次被验证。** 路由对 4 个扫描件全部正确识别为 scan。公式：扫描件 5/5 exact，**劣化版（倾斜 0.4°+噪点+JPEG60）同样 5/5**——加的劣化对两个模型都没影响。表格是另一回事，见 E 栏。
>
> **④ 按页路由：被数据否掉，撤销。** 我估计"物理论文大部分页是散文，按页路由能省 2–3 倍"。实测：**安全判据（数学字体）下只有 9.4% 的页没有数学**——LaTeX 排版里几乎每页都有行内 `$x$`。换成"有没有编号行间公式"号称 67.7%，但**探测器在 9 页验证集上漏掉 4 页**（44% 漏检），每一次漏检就是一页公式被压扁。**估错了约 25 倍，五分钟测出来，省了一周。**
>
> **⑤ 图片提取器确认可独立使用**：`extract_figures` 不碰 OCR，toric 4/4、hubbard 8/8 且都带标题，**0.3–0.6 秒**。扫描件上是 0（靠标题文字锚定，这是它的边界）。所以"给 rung 3/4 也接上"是纯解耦。
>
> **⑥ 复读是刀锋上的，而且可重现。** 同一张扫描页两种渲染：4,200 字符干净 vs **49,561 字符、正文重复 20 遍**。重复在这一页的 OCR 文本内部（14 行一周期），不在组装阶段；**同一路径连跑两次字节级相同**。所以它不是"这页太难"，是"这页在边界上"，而边界另一侧就在旁边（切图后干净）。**这让"门触发重试"从兜底变成大概率有效**——记为 D8。
>
> **⑦ 一次操作事故，值得记。** 第一轮 MinerU **7/7 全部失败**：上传成功、服务端转换完成（`Status: done`）、下载失败。不是 MinerU 挂了——结果放在 `cdn-mineru.openxlab.org.cn`，而整个 `openxlab.org.cn` 域名当时在这台机器上不可达（代理隧道建立成功 `HTTP 200`，TLS 握手收不到回应）。用户改了代理规则后 7/7 全通。**7 次配额白花。** 而 `mineru.py:136` 下载**没有任何重试**，报错只说 `SSLError`，不说"转换已完成并已计费"——rung 4 现在是主力路线，这是单点故障。
>
> 2026-08-23（日·四）rung 2 的另一篇也修好了，病因有三层，其中一层是我自己当天埋的。用户「继续 debug」。
>
> `cond-mat/0001002`（2000 年投稿，用的还是 `\documentstyle` 而不是 `\documentclass`）原来 pandoc **一个字都不吐**。拆开是三个独立问题：
>
> **① `true` 单位前缀，pandoc 的尺寸解析器不认识，而且有两种坏法。** 实测最小对照：
>
> ```
> \vskip 0.3truein    （数字与单位之间无空格）→ 硬报错，整篇返回 0 字符
> \vskip 0.3 truein   （有空格）             → 不报错，但把 "truein" 当正文吐出来
> \vskip 0.3in                              → 干净
> \vspace{0.3truein}                        → 干净（花括号形式没事）
> ```
>
> **一个炸掉、一个静默污染**，后者和 B2 记的 `\hline` 漏成 `height` 是同一类。归一化之后：**0 字符 → 79,090 字符**。
>
> **② `\epsfbox` / `\epsfig`——graphicx 赢之前的图片宏。** pandoc 两个都不认识，所以文档即便能解析，13 张图对它也是隐形的。翻译成 `\includegraphics`，那正是它们的含义。顺带一个省事的地方：`\newcommand\frm[1]{\epsfig{file=#1,...}}` 这类自定义宏 pandoc 自己会展开，所以只改**定义里**那一处，所有调用点就都好了。
>
> **③ 我自己当天引入的 bug，靠第二篇论文才暴露。** 上一轮的 unwrap 规则匹配到了 `\newcommand\frm[1]{\includegraphics{#1}}`——那是宏的**定义处**，不是包装。unwrap 之后变成 `\newcommand\includegraphics{#1}`，**把下游一切都依赖的那个命令重定义了**，而 pandoc 在 200 行之外才报错。加护栏：紧跟在 `\newcommand` / `\renewcommand` / `\providecommand` / `\def` / `\DeclareRobustCommand` 之后的名字不是包装。**写测试时又抓出护栏正则本身写错**——`(?:re|provide)?newcommand` 生成的是 `\providenewcommand`，真实的宏叫 `\providecommand`。
>
> 结果：`cond-mat/0001002` **12 张图全部落盘、0 未解析**，78,785 字符、46 个行间公式、`truein` 零泄漏。EPS 因为这台机器没装 Ghostscript 是原样拷贝（已有的降级路径，会打印提示）。
>
> **门如实报了剩下的问题，这正是它该干的事**：`leftover-tex`（10 段裸 TeX——这篇用 LaTeX `picture` 环境手画杨氏图，pandoc 真转不了）和 `repetition-loop`。审阅者据此可以拒绝、降到模型路线，而不是拿到一份"看起来完整"的东西。
>
> **方法记一条：一个真实样本修完不代表规则对，第二个样本才暴露了规则会砸掉宏定义。** 而第二个样本就写在同一条 E 栏笔记里——那条笔记记了两篇，我上一轮只查了一篇。
>
> 测试 1032 → **1053**（+21）。CI 环境 1045 绿 + 8 skip。
>
> 2026-08-23（日·三）rung 2 丢图不是路线的性质，是 pandoc 丢宏。用户质疑「读 tex 源码怎么会丢图和表？arxiv tar.gz 讲道理应该是无损的」。**这个质疑是对的，而且原来那条记录把病因归错了地方。**
>
> tarball 确实无损：`2401.00506` 的包里 9 个文件，6 张图片一张不少（`3dplot.pdf` `disp.pdf` `jjc.pdf` `plot1.pdf` `plot2.pdf` `sbs.jpg`）。问题在转换。
>
> **机制（pandoc 3.9 实测，最小对照）：**
>
> ```
> \includegraphics[width=0.5\textwidth]{sbs}     -> ![](sbs.jpg){width="50%"}   图在，连扩展名都自己找到了
> \subfigure[]{\includegraphics[...]{disp}}      -> <figure><figcaption>…       图没了，标题还在
> ```
>
> **pandoc 的 LaTeX reader 遇到不认识的宏，连宏带参数一起丢。** 而 `\subfigure`（`subfigure` 包）和 `\subfloat`（`subfig` 包）正是做多面板图的两种标准写法——物理论文的图基本都是多面板的。那篇论文 6 个 `\includegraphics` **全部**套在 `\subfigure` 里。**而且标题活下来了，所以输出读起来是完整的**——又是这个失败形状。
>
> **修法不枚举包名。** 枚举只能撑到下一个包出现。规则是结构性的：**任何一个"花括号参数里含 `\includegraphics`"的宏调用，都把里面的内容拿出来、把壳丢掉**。已知 pandoc 认识的（`\caption` `\href` `\label` `\begin`）排除在外——拆它们是丢信息，不是修。
>
> **写测试时发现自己的实现有真缺口，而这个缺口是靠"先查 pandoc 到底怎么做"补上的。** 我最初只看**第一个**花括号组，于是 `\resizebox{2cm}{!}{\includegraphics{…}}`（图在第三个组）漏掉了。实测四个盒子宏：**`\resizebox` `\scalebox` `\makebox` `\raisebox` 全都丢图，只有 `\parbox` 不丢**——所以"按宏的形状猜"是错的，规则改成**读完连续的花括号组、取真正含图的那一个**。
>
> 实测结果：`convert()` 走完整流程，`Figures: 6 embedded into images/, 0 unresolved`（原来是 0/6），6 个文件真的落盘（PDF 转成 PNG、JPEG 直接拷），markdown 里 6 条引用，gates 无 finding。
>
> **顺手撞出第二个 bug，比第一个更隐蔽。** 表格没丢——但 `cmd.extend(["-t", "markdown"])` 让 pandoc 输出的是 **simple table**（空格对齐、一行短横线），而：
>
> - **Obsidian 和所有 GFM 阅读器都不渲染它**，所以一张表进了库其实是一段乱码似的正文；
> - **`tables-dropped` 门也看不见它**，于是在"Best"评级的这条路线上发出最响的误报。
>
> 一个默认值，同时造成一次真实丢失和一次误报。改成 `-t markdown-simple_tables-multiline_tables-grid_tables+pipe_tables`。太复杂而无法用 pipe 表达的表会退化成裸 HTML `<table>`——**阅读器和门现在都认得**（门认 HTML 是当天早些时候为 rung 5 加的，正好接上）。
>
> **这一轮的方法教训**：用户说"讲道理应该是无损的"，那就该去把 tarball 拆开数文件，而不是复述笔记。**笔记记的是现象（丢了 6 张图），归因（"这条路线会丢图"）是当时顺手加的，没查。** 现象是真的，归因是错的，而错的归因写进 ROADMAP 之后就变成了"已知事实"。
>
> 测试 1009 → **1032**（+23：`test_tex_figures.py`）。CI 环境 1024 绿 + 8 skip。
>
> 2026-08-23（日·续）rung 5 拿回了表格，而 A9 记的病因是错的。用户「开始优化」。
>
> **A9 写的是"glm-ocr 把表格全丢了（151 行 → 0）"，归因为模型能力。归错了：是提示词。** MAGI 发给它的是 `Text Recognition:`（`ocr_engine.py:42`），它自己的 OCR 提示词，一个字没提结构。同一个模型、同一批页、同一份权重，换成一句提到 **Markdown** 的提示词，表格就出来了，而且单元格连符号都对：`x^{-2}y^{-5}`、`(4,-6)`——这些正是整页模式的云端模型会编成 `x^{32}` 的地方。
>
> **提示词的措辞几乎不起作用，只有"提到哪种记法"起作用。** 扫了 7 个提示词，只回来 3 份不同的字节：「输出每一行，不要提前停止，不要用省略号」和 `Text Recognition:` **md5 完全相同**；「先数有多少行再全部转写」和「一行一行输出，用 ` | ` 分隔」也完全相同。能解锁表格的只有 **Markdown** 和 **HTML** 这两个词。所以代码里那句提示词是**实测过的那一句**，不是它的改写版——注释里写明了这一点。
>
> **A10（转到一半就停）也不是预算问题。** `num_ctx` 开到 16384 / 32768 / 65536，`num_predict` 设到 8192，**输出逐字节相同**（4201 字符、25 行）。是模型自己发 EOS，覆盖了大约一半的表就认为写完了。
>
> **所以杠杆是"一张图里有多少页面"，不是"分辨率"，而这两个很容易搞混。** 把**同一页的图像**缩小，它确实写得更多——0.7 MP 吐出 56 行（真实只有 49 行）——但只有 12 行是对的，多出来的是编的。把**同分辨率的一块**给它，写得更多**而且读得对**。前者正是这整条阶梯要绕开的失败方式，在本地模型上重现了一遍。
>
> | toric p4，49 行表 | 时间 | 两格全对 |
> |---|---|---|
> | 整页 + 原提示词 | 16.0s | 0 |
> | 整页 + 新提示词 | 16.0s | 24（49.0%） |
> | **左右两块 + 新提示词** | **30.1s** | **49（100.0%）** |
> | 3 条横切 | — | 40（81.6%）——切穿了表格行 |
>
> 对照：DeepSeek 整页 12 行（24.5%），2×2 切图 49 行（100%）。**本地模型追平了，而公式本来就高一档**（exact 66.7–68.9% vs 60.0%）。**这条云端路线因此不需要了**，它只剩"不会崩"和"不占显卡"两个优势。
>
> **只对确实含表格的页切图**，判据是 PyMuPDF 在源 PDF 里找到表格。这不是拍脑袋：九页样本里，它在 3 个"切图有用"的页上报表格，在 6 个"切图无用"的页上报 0——**包括那个切了会更糟的页**（`defects_61` 竖切跑了 127 秒、吐出 32,785 字符的复读，整页只要 6.4 秒 / 559 字符）。两类页被这个判据完全分开。顺带确认：**那次复读被现有的门接住了**——`output-inflated`（14.4×）、`unclosed-environment`、`leftover-tex` 三条全响，A10 那批门正好是这个方案的兜底。
>
> **过程里撞出两个真实缺陷，都不是设计问题，是接缝问题。**
>
> ① **切片存成 JPEG，等于把同一批像素有损压两次**（tiler 存一次，`_preprocess_image` 出去时又编一次）。后果不是"稍微糊一点"：双重编码的右半块回来 **146 个 `<tr>`**，而那里只有 25 行——**模型在糊掉的像素上编行**。改成 PNG（无损）之后同一块回来 25 行且全对。**有损步骤只能有一个，放在本来就负责它的那个边界上。**
>
> ② **模型不关表格就继续写正文**，于是整页最后那两段散文变成了表格行——读起来不知所云，而且**看上去像数据**。规则不是"最后一行像散文就摘掉"（中间夹了一行公式会把回溯卡住，把上面那句散文孤零零留在表里），而是：**表格是有列的**，所以末尾连续的单格行整体不属于表格；至少要有一行读着像散文才动它，否则单列表还是表。
>
> **缓存必须跟着配方走。** `.temp/page_N.json` 只按页码存，旧缓存是旧提示词、没切图的产物；直接复用等于**这个修复只对"从没跑过的人"生效**。缓存里现在记 `model | prompt 哈希 | 切没切`，对不上就重跑。
>
> **`tables-dropped` 门同时放宽认 HTML 表**。转换发生在 OCR 路线内部，门本来看不到 HTML；但一个只认 markdown 的门，会把满是表格的文档判成"表格全丢了"——在唯一真正需要抓这个错的路线上，发出最响的误报。
>
> 测试 957 → **1009**（+52：`test_ocr_tables.py` 26、`test_ocr_tiling.py` 26）。CI 环境 1001 绿 + 8 skip。端到端复核用的是 `magi ingest ocr` 本身，不是脚本：真实一页 **49/49 行、两格全对**，输出里没有残留 HTML。
>
...
| **DeepSeek 视觉**（`deepseek-v4-flash-vision-exp`） | **不进阶梯。理由是"被 MinerU 全面压住"，不是"质量不行"**——这一点当天先写错过一次 | 同九页、同评分器（修正后）：DeepSeek 2×2 切图 **42 exact / 7 wrong**，MinerU **42 exact / 5 wrong**。同样的正确量，MinerU 错得更少、更快、表格不用切图，而且已经在阶梯里。另：整页发**必然**编数值（`x^{-4}` → `x^{32}`），必须带切图器；思考默认开启且与输出共用 `max_tokens`，四页里三页返回空字符串 |
| **未测**：DocMind 在扫描件上的表现，现在**优先级更低** | 它要挑战的位置（rung 5 替代）已经被证明不是瓶颈：扫描件先走 rung 4，而 rung 4 在扫描件表格上 49/49、公式 5/5 | 见 2026-08-23（日·五） |
| **在页面图像里靠横线找表格**（给扫描件补切图判据用） | **否掉。两条都不成立。** ① 在**原生**页上就分不开：29 个真表格页里 **18 个**没有一条跨 30% 页宽的横线（booktabs、无框线表），而 104 个非表格页里 **44 个**有（图的坐标轴、页眉线）。② 更要命的是**它恰恰在该起作用的地方崩掉**——同一张扫描的表格页，干净版检出 27 条横线，倾斜 0.4° 的劣化版检出 **0** 条。一个只在不需要它的时候才准的判据 | 2026-08-24 实测，4 篇论文 133 页 + 4 个扫描件。替代方案见当天条目：改成问转写本身 |
...
| ~~D3~~ | ~~**派生数据 vs 原创数据没有统一标记**~~ **约定已落地 2026-08-24** | 三类写进 `core/durability.py` 并配测试：ORIGINAL（人做的，`raw/` `wiki/` 两个 config，**以及工作区之外的 `registry.json`/`settings.json`**）、DERIVED（命令会再做一遍）、TRANSACTIONAL（`output/ingest/`，记的是人的决定，但它指的产物已经在 `raw/` 里了）。`unknown` 是一个真实答案不是失败——**没人决定过的路径要报"没决定"，猜是怎么把 `output/ingest/` 塞进"删掉 output/ 重来"那句话里的**。同一轮 P5 也把 ORIGINAL 那一侧的写法补齐（原子写 + 锁住整段事务） |
| ~~D4~~ | ~~**WebUI 在用 JS 重新实现后端逻辑**~~ **摄取那一侧已收敛 2026-08-24；JS 那一侧仍在** | 原记录说的是 JS 重实现 Python。实际找到的更糟：**两份 Python 实现，其中一份是错的**——驳回降级（CLI 继承 `source_type`，WebUI 硬编码 `"arxiv"`）和未决议计数（`i.decided` vs `not i["decision"]`）。而且这两处**就在为了 D4 才共用 `review_order` 的那个功能里面**。三个判断收进 `ledger`（`requeue_next_rung` / `undecided` / `blocking_commit`），两条结构性扫描守住。**剩下的仍是 JS 侧**：`countTasks()` vs `bd_status_summary()`、i18n 抄了一遍 CLI 文案。判据不变：**一段 JS 在做判断而不是渲染，它大概率该在 Python 里** |
| **D9（新）** | **门后面的正则手术没有守卫**——一段正则在所有门之后重写 markdown，因此它做什么都没有下游会发现 | 三次了：⑧ `MarkdownCleaner._fix_latex` 把 19 行表格劈成两行（引擎 53 个 pipe 行 → 文件 72 个）；B2 的 `\hline` 漏成 `height`；评审又找到 `format_math` 把 `$$A$$ and $$B$$` 熔成一个带非法定界符的块、`image_refs` 改到 alt 文本而不是 URL。**"哪些跨度不能碰"的判断此前各写了一份，而 OCR 那份认表格不认代码块、KB 那份反过来**。已抽成 `core/md_blocks`，三处共用。**债没还完的部分**：还没有一条"进出计数必须一致"的端到端哨兵覆盖全部四条路线（`test_md_blocks` 只覆盖原语） |
| D5 | **Skill 是没有测试的代码，而且单位字符代价最高** —— **最小版已做 2026-08-24，行为仍然测不了** | 已做：12 个会 fan out 的 skill 补上 `## Rules` 块（此前只有 `wiki_inbox` 有），三条共用的"绝不"是这个项目**真正付过钱**的三种失败——没数过的 fan-out（一篇 99 页论文烧掉一周额度）、sub-agent 去问一个它结构上问不出的问题、把部分结果报成完整结果——外加每个 skill 一条自己的。四条文本层检查守住，另加一条**事故本身**的检查：任何提到按页视觉转写的文件，必须同时写明代价并要求人显式同意（`wiki_inbox` 是直接禁止，比设门更强，测试认这个）。**仍然做不到的是查行为**：这些只能证明字写在文件里，不能证明 agent 照做 |
| D6 | ~~**上游依赖承重且无版本承诺，但没有降级表**~~ **表已写 2026-08-24，`docs/degradation.md`** | 八行（arXiv HTML / ar5iv / e-print tarball / pandoc / MinerU / Ollama / S2 / PyPI），每行写明退化成什么、**用户看到什么**、实现在哪个文件。8 条测试守着：每行都要说用户看到什么，每个点名的代码路径必须还存在，四条关键 fallback 逐个钉住。表自己写了三条边界：**没有一行是对着真实宕机跑过的**（只有 MinerU 那次是真事故）、部分可用没建模、没有上游可以钉版本 |
| ~~D8~~ | ~~**门只会发现，不会动手**~~ **已修 2026-08-23（日·六），限 rung 5 的复读** | 12 道门全是检测，**整个系统唯一的自动反应是"人拒绝 → 降一级"**。所以每一次检测都要花掉一次人的注意力。而（日·五）实测：复读在同一渲染下**逐字节可重现**，换一种切法就干净——**它是可修的，不只是可报的**。"这一页复读了"应该意味着"这一页换个参数重跑"，而不是"去问人" |
...
- rung 2（tex/pandoc）**两篇老论文暴露的缺陷现在都查清并修好了**（见 8-23 日·三、日·四）：一篇是 `\subfigure` 包装，一篇是 `true` 单位前缀 + `\epsfbox`。两篇现在都整篇转出、图一张不少。**但两篇不是样本**——真实成功率仍未测，等 rung 1 上线后看 rung 2 的实际流量
- arXiv 覆盖率测量**没取到** pre-2000 的原生 HTML 桶和非物理学科桶
- ~~本地 OCR（rung 5）在数学上的质量从未测过~~ **已测，2026-08-23，见当天条目**：公式好（干净页 39 行里 3 行真错），~~表格全丢（A9）~~**表格是提示词丢的，不是模型丢的，已修（见 8-23 日·续）**，会复读（A10，仍在）。~~**仍没测的是扫描件**~~ **已测，2026-08-23（日·五）**：扫描件上公式质量**不降**（5/5 exact，倾斜+噪点+JPEG60 的劣化版同样 5/5）；~~但表格塌到 10/49~~ **表格那半句是评分器的错，见 2026-08-24 条目**
- ~~**MinerU 的图没有对着我们自己的 ground truth 查过**~~ **已查，2026-08-24。不用动。**
  8 张图的对照件上 MinerU 出 11 个文件，看着像碎片化，其实是**把多面板图拆成各个面板、
  每个面板配自己的子标题，而主标题 `FIG. 3.` 也在**。8 张一张不少，粒度比我们的整图裁剪更细
- DocMind 在扫描件上的表现（见 C）
- **rung 2 的 `picture` 环境**：cond-mat/0001002 用 LaTeX `picture` 手画杨氏图，pandoc 转不了，
  留下 10 段裸 TeX。门报出来了，但没有"这一页降级到模型路线"的机制（见下面的架构债）
- ~~**切图对扫描件有没有用，没测。**~~ ~~**已测（日·五）：有用，而且是数量级的**~~
  **两次都记错了，2026-08-24 修正并修好。** 那个 20.4% 是评分器打出来的（它要求 `[[n,k,d]]`
  双方括号，模型写的是 `[n,k,d]`）。真实数字：**整页在原生 PDF、干净扫描件、劣化扫描件上都是
  24/49，一模一样**——所以这从来不是扫描件的问题，是模型的定额，和 A10 记的 EOS 是同一件事。
  修法也不是"给扫描件做图像判据"（那条已实测否掉，见 C 栏），而是问转写本身。见当天条目
