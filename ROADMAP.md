...
- rung 2（tex/pandoc）的成功率只在**两篇老论文**上试过。~~一篇静默丢 6 张图~~ **那篇已查清并修好（见 8-23 日·三），是 `\subfigure` 包装导致 pandoc 连宏带参数丢掉，不是路线的性质**；另一篇 pandoc 死在 `\vskip` 仍未处理。完整样本等 rung 1 上线后看 rung 2 真实流量再说
