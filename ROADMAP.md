...
>
> 最后更新：2026-08-23 · 当前阶段：**v1.12.4 已发布**。此前：M0–M9 全部完成。
>
> 2026-08-23（日）DeepSeek 视觉模型实测，以及顺手修掉的三条。用户「探索一下 pdf 视觉模型的新管路……别跑太多，注意控制成本」「10RMB 以内都能接受」。**实际花了 0.41 元。**
>
> **结论先写：值得进阶梯，但必须带切图器；整页发是错的。** 不是慢一点差一点，是把"读不出来"变成"编一个"。
>
> 方法：`deepseek-v4-flash-vision-exp`，样本是 rung 5 用过的**同九页**、同一批 PNG、同样的 2 MP + JPEG-95 前处理、temperature 0、MAGI 自己给通用视觉模型准备的那句提示词（`ocr_engine.py:54`），评分用同一个 `grade2` 归一化器。**变的只有模型和分块方式。**
>
> **三个结构性事实。** ① **不吃 PDF**，只接受 JPEG/PNG/GIF/WebP——所谓"PDF 摄取"仍然要我们自己渲染页面，和 rung 5 同一套前处理，不省事。② **整页被压成约 380 个视觉 token**（实测 `prompt_tokens: 391`，其中约 55 是文字提示），等于把 A4 页当 ~95 DPI 读。③ **思考默认开着，而且和输出共用 `max_tokens`**——最早四页里有三页把 8192 token 全烧在 `reasoning_content` 上，`content` 返回**空字符串**、`finish_reason: length`。加 `"thinking": {"type": "disabled"}` 之后九页全部正常，而且更快（toric p4：66.6 s 空结果 → 26.6 s / 6489 字符）。**这条是产品级的坑：一个默认开着的功能，把一个转写任务变成了静默返回空页。**
>
> **切图是决定性的。** 同页、同模型、**同样壁钟时间**（27.5 s vs 28.2 s），输入 token 391 → 1455。按 `[[n,k,d]]` 三元组对齐、逐行比对两个多项式单元格：
>
> | 页 | 表格行 | 整页两格全对 | 2×2 切图 |
> |---|---|---|---|
> | toric p4 | 49 | 12 (24.5%) | **49 (100%)** |
> | toric p5 | 22 | 0 (0%) | **22 (100%)** |
> | toric p13 | 46 | 2 (4.3%) | ≥36 (78%+)，余下 9 行是真值抽取错位、不是模型 |
>
> 公式（按公式**行**计）：整页 exact 38.8% / 65 行，切图 **exact 60.0%**，本地 `glm-ocr:q8_0` 68.9% / 45 行。
>
> **和本地模型的关系不是谁更强，是失败方式不同。** 在 glm-ocr 正常工作的七页上**两者吐出的公式行数完全一样**（4/5/4/9/5/5/2），而 glm-ocr 每行更准。差距全部来自 glm-ocr 崩掉的两页：`defects_61` glm-ocr 只吐 **1** 行（复读后截断），DeepSeek 切图吐 **14** 行。加上表格——glm-ocr 全丢（A9），DeepSeek 切图基本全对。
>
> **整页模式的失败方式比 glm-ocr 危险得多。** glm-ocr 坏起来**看得见**：复读、截断、跑 181 秒。DeepSeek 整页坏起来**看不出来**——版面正确、markdown 干净、表格整齐，只有数值是编的：真值 `x^{-4}` / `x^{-3}y^{2}` 出来是 `x^{32}` / `x^{4}y^{8}`，负号丢了，指数换成一组"看着像物理"的 2 的幂。**审阅者扫一眼会直接通过。这正是当初否掉 DocMind 的同一个理由，只是伪装得更好。**
>
> 成本与位置：切图后每页约 1455 in / 2900 out，**一篇 70 页论文 1.1 元（非高峰）/ 2.2 元（高峰）**；约 28 s/页串行，但云端可并发（本地 GPU 不行），70 页 8 路并发约 4 分钟。位置是 rung 4（MinerU）的替代或补位：不用排 token、没有 30 分钟超时、不占用户显卡。**本地 glm-ocr 仍是唯一的离线路线，保留。**
>
> ---
>
> **同一轮顺手修掉的三条，其中两条是对本文档的更正。**
>
> **A12（新）：`magi setup --full` 报告成功但什么都没做。** `--full` 只给 `profile` 赋值，而 `feature_enabled` 只允许 `profile: kb-only` **否决**——所以 `optional_features["tasks"]` 一旦是 False，翻 profile 翻不动。命令打印"profile set to full"，任务追踪保持关闭，**从输出里看不出来**。这正是 D2 预言的漂移。三个写入点全部改走 `set_feature`。
>
> **写测试的方式带出了第二个写入点。** 我加的不是"再测一次这个行为"，而是一条**结构性**测试：`features.py` 之外任何文件出现 `["profile"] =` 就失败。它当场抓出 `setup_cmd.py:343` ——那处写得是**对的**（两处一起写），但它是第二个必须记得同步的地方，而第三个就忘了。**问"还有谁能写这个"，比问"这个行为对不对"抓得多。**
>
> **B1 复测，比原记录更难看。** 原记录说 55 页吐 403 张。复测：一篇 23 页、**只有 4 张图**的论文吐 **117 个文件**，102 张小于 200×200，最小的 40×24——**是把单行行间公式渲染成了图片**，而且全部内联进 markdown，读起来像这篇论文有 117 张图。`image_size_limit` 帮不上忙：`to_markdown` 声明是 `(*args, **kwargs)`，**用不到的参数被静默忽略**，0.05 和 0.25 输出逐字节相同。**"我传了这个选项"不是"这个选项起了作用"的证据**——同一个陷阱当初也吞掉了 table 参数。已默认关闭并加 finding 明说。
>
> **B2 复现不了，而且不是版本升级修好的。** 装的还是记录 B2 时那个 pymupdf4llm 1.28.2。全框线表、booktabs、完全无框线、以及一篇真实论文的全部 10 张表（census 205 行 → markdown 285 个 pipe 行），**全部正常出 markdown 表格**。原证据里 `\hline` 漏成 `height` 指向某一类特定文档，而那份文档找不到了。处理方式不是删条目，是**把当前的正确行为钉成测试**——真回归了由测试说话，不由一条没人复测的笔记说话。**B/C/D/E 里的条目都有保质期，而保质期是隐式的。**
>
> **A13（同一轮，用同一个问法找到的）：数学门分不清"没有数学"和"没看成"。** `textlayer.inspect` 读字体表是 best effort——`except Exception: pass`。**如果每一页都抛异常，`fonts_seen` 是空的，`has_math` 就是 `False`**，于是一篇数学论文被判成散文，走上唯一会把公式压扁的那条路线。**而这跟 v1.12.3 修的那个 bug 是同一个形状**：那次是把"这台机器能不能跑"混进了"这篇文档该走哪条路"，这次是把"没看成"混进了"没有"。
>
> `TextLayer` 增加 `math_known` 字段，`route_here` 变成三个条件。全部页面都读失败时 `math_known=False`，`reason` 直接说"font table could not be read on any page"——否则一篇散文 PDF 跑去 OCR，看起来像路由 bug 而不是坏文件。**单页失败仍然按 best effort 放过**，只有全军覆没才算"没看成"。
>
> 测试 941 → **957**（+16：features 6、batch 6、textlayer 4）。CI 环境（隐藏 pymupdf4llm）949 绿 + 8 skip。
>
> 2026-08-23（六）怎么读自己的流量数字——记方法，不记结论。用户注意到 clone 和 PyPI 下载"很多"，追问"是不是真有这么多人"。查下来**四条判读法，其中一条是我当天判错又收回的**。
>
> **① 先减掉自己的 CI。** `actions/checkout` 会记进 GitHub traffic 的 clone 统计，而**每个 job 是一台独立 IP 的临时 runner，所以它同时贡献 clone 数和"唯一 cloner"数**。8/20 那天 31 次 workflow 运行、**67 个 job**，对应 399 clone / 101 唯一 cloner——CI 大概占了其中一半。这条我一开始完全没想到，是顺着"为什么只有这个仓库有 CI"才发现的。
>
> **② PyPI 看 `system` / `python_minor` 里的 null 占比。** 真正的 `pip install` 一定在 User-Agent 里报出 Python 版本和操作系统，两个都是 null 就不是 pip（是 curl/wget、爬虫、扫描器、缓存代理）。实测：`without_mirrors` 的 1752 次里 **1317 次（75.2%）两项皆 null**，真实 pip 安装约 **435** 次（Linux 321 / macOS 70 / Windows 44）。注意 `without_mirrors` 只滤掉了镜像客户端，**没滤掉这 75%**——shields.io 徽章上的 1.8k 就是这个未滤的数。
>
> **③ clone/访问 比例，对"靠 clone 安装"的仓库毫无意义——这条我判错过。** 我先据此断言"93% 的 cloner 是机器人"，理由是人类会先看页面再 clone。**用户用一个反例推翻了它**：Wikify 时期还没有 PyPI 包，clone 一样多。查最早的 README，第一行就是 `### Step 1: Download or Clone the Repository`——**这个仓库从第一天起，安装方式就是 clone，装它的人根本不需要打开页面**；加上作者分享的是命令而不是链接。**前提错了，推论再顺也没用。**
>
> **④ 要区分"传播"和"自动化"，就找一个没有动作的窗口。** 两者在有动作时长得一样，在没有动作时完全不同：口耳相传会衰减，自动化跟着发版走。现在的三天数据（8/20 首日 1111、之后稳在 ~300/天）**分辨不了**，因为 8/20 当天一次性上传了 21 个版本。**唯一干净的一天是 8/19：CI 跑了 0 次、PyPI 还没发，却有 31 个唯一 cloner**——那是一个不属于任何已知机制的外部流量基线。
>
> **时间线（可查，不是猜）**：8/17 `Merge magi-rebuild: Wikify -> MAGI` 改名 + 变成 Claude Code 插件；8/18 连发 v1.0.0/1.1.0/1.1.1；8/20 首个 PyPI 版本 **同一天** 31 次 CI 运行。三件事各自都会引来抓取，叠在一起就是那条从零起跳的曲线。对照组也很干脆：同一账号另外两个 public 仓库 clone 分别是 **2** 和 **1**，它们不在 PyPI、不是插件、不靠 clone 安装、没有 CI。
>
> **为什么记这个**：这些数字会反复被看到，而**四条里我错了一条**。错的那条不是算错，是**前提没检查**——"人类会先看页面"对绝大多数仓库成立，对这一个不成立。下次再看这类数据，先问"这个仓库是靠什么被安装的"。
>
> 2026-08-23（五）配置终于有了"你的"这一层，入队终于幂等 (v1.12.4)：用户「mineru 那个也做了。我机器就留一个全局的就行。a11 也做了」。
>
> **D7：配置从"二选一"改成"两层"。** `find_config_yaml` 原本是**首个命中即返回**——向上走撞到第一个工作区/hub 根下的 config.yaml 就用它，只有一路走到头才回退 `~/.config/magi/config.yaml`。而**每个 `magi init` 出来的工作区根下都有 config.yaml**，所以那条全局路径对任何"有工作区的人"（也就是所有人）来说从来没被读到过。属于个人而非某个课题的东西只能往每个库里抄一份：实测这台机器上同一个 402 字符的 MinerU JWT 存了三份。
>
> 现在是 defaults → 用户层 → 工作区层 → 环境变量，**逐键合并**。工作区仍能覆盖任何一项，但不必再重复它不想改的东西。
>
> **迁移过程当场教了一课，而且这一课比 bug 本身更值得记：** 我先把三个工作区的 token 设成 `""`，结果全部读不到——**显式的空字符串是一个值，它正当地覆盖了底层。"空"不等于"没有"。** 这不是分层的 bug，是分层在正确工作。
>
> 但顺着它摸出一个**系统性得多**的问题：`magi init` 的模板里写着 `mineru_api_token: ""`——也就是说**今后每个新建的工作区，都会用一个空字符串盖掉用户设在全局的 token，并且悄无声息**。模板改成把那一行注释掉（"absent, not empty"），并写了测试直接钉住模板：`ocr:` 段里不许出现未注释的 `mineru_api_token`。顺带模板里 `dpi: 130` 与真实默认 150 对不上，一并改了——和当天在 `wiki_ingest_ocr/SKILL.md` 改掉的是同一处陈旧说法。
>
> **还揪出一个隐蔽的隔离漏洞**：`find_config_yaml` 硬编码 `~/.config/magi`，而 `kb_registry` 认 `MAGI_CONFIG_HOME`。也就是说**一个隔离了注册表的测试，仍然在读开发者的真实配置**——这种泄漏只会表现为"莫名其妙地通过"。`config_home()` 收进 `core.workspace` 作为唯一实现（放这里而不是 registry，因为 registry 本来就 import workspace，反过来会成环），registry 改为委托。
>
> **A11：入队幂等，判据是"还在等着的"而不是"这辈子见过的"。** 三条豁免各自都是真实工作流，全部写了测试：① **retry**（拒绝后降级重排队，跟原请求去重会废掉降级路径）；② **已被批次领走的**（arXiv 出新版本，重新摄入合理）；③ **另一个库**（不同目的地 = 不同请求）。还有一条容易漏：**retry 自己被豁免，但它也不能成为别人去重的对象**——否则一次拒绝会把之后所有对该论文的点击都悄悄吞掉。
>
> **原来有一条测试明确钉着"允许重复"**，理由是「去重是审阅者的决定，不该在这里悄悄丢掉」。**那个反对意见针对的是"悄悄"，不是"合并"**——所以实现返回已有的 req_id，同时 CLI 打印 `already queued`、端点返回 `already-queued`、弹窗说「已经在队列里了」。测试改成钉住这一点：合并了，**并且说出来**。说"已入队"而其实没有，比重复更糟——重复至少是看得见的。
>
> **顺带**：README 的下载徽章显示 `rate limited by upstream service`。查证是 **shields.io 全局在限流**（`requests`、`numpy` 一模一样），不是我们的包。pepy.tech 能用但报 6k——那是**含镜像**的数，而实测 77% 是镜像。**挂一个明知虚高 3.4 倍的数字，比偶尔显示"不可用"更糟**，所以保留 shields 并加 `cacheSeconds=86400`。
>
> 测试 923 → **941**（+18：`test_enqueue_idempotent.py` 10、`test_config_layers.py` 8），CI 环境（隐藏 pymupdf4llm）938 绿 + 3 skip。
>
...
>
> **五个新闸门，全部来自当天量到的失败形状。** 关键是先补上了"进去多少"这一半——`textlayer.census()` 数源 PDF 的字符、表格、表格行数，闸门本身仍是纯函数（沿用 `check_figures(referenced, resolved)` 早就定下的分工：调用方数数，闸门判断）。`arxiv-html` / `tex` 两条路线没有本地 PDF，census 为空，需要它的检查**自动噤声**——"产物比源短"在没有源文档时不是一个有答案的问题。
>
> | 闸门 | 抓的是 |
> |---|---|
> | `tables-dropped` | 源有表、产物无表格标记。**表题会正确转出**，所以产物读起来像完整的一页 |
> | `unclosed-environment` | `\begin{X}` 与 `\end{X}` 不平衡。模型停在结构中间就留下这个，渲染时会吞掉后面所有内容 |
> | `output-much-shorter-than-source` | 产物字符数低于源的 80%。前两条的通用版：不需要有表格、也不需要留下未闭合环境 |
> | `output-inflated` | 产物超过源的 4 倍。上一条的另一端——**页面读不出比它本身更多的字** |
> | `repetition-loop` | 一段 200 字符的**有变化的**段落出现多次 |
>
> **`repetition-loop` 的第一版是错的，而且是自己的测试抓出来的。** 它只看"200 字符的块重复出现"，于是 `"word " * 300` 这种测试夹具直接触发——**现有测试里 `run_all(_doc(LONG_BODY)) == []` 当场变红**。这不是夹具的问题：真实文档里也有一列相同的数值、一串格式雷同的引用。改成用 KMP 失败函数求**最小周期**，周期 < 40 字符的判为均匀填充、不算循环。而当天量到的 27,281 字符 `& 0 & \cdots` **恰恰是均匀填充**，被这条规则放过——所以它交给 `output-inflated` 接（17.9 倍），两条规则各管一种形状。顺带发现扫描步长 50 对短重复单元太粗、且 frontmatter 后的空行会让每个窗口错位，都修了。
>
> **验证方式是拿当天真实产物跑，不是只跑合成用例。** 10 个真实页：5 个坏页全部命中（toric p4/p13 → `tables-dropped` + 短于源；defects p61 旧 Ollama → `repetition-loop` + `output-inflated` + `unclosed-environment`；defects p61 q8_0 → `unclosed-environment` + 短于源），**5 个正常页一条都不响**。零误报——一个什么都报的闸门不是闸门。
>
> 文档同步：两份 guide、两份 readme（含根目录两份，字节一致）、`wiki_ingest_ocr/SKILL.md`。那个 skill 里顺手改掉一处**早就不对的说法**——它写"默认 glm-ocr at 130 DPI"，而配置里的默认一直是 150。
>
> 测试 879 → 902（+23，`tests/test_gates_ocr.py`）。
>
> 2026-08-23（续）量化实测，以及两个被我自己测出来的假象。用户提议试 Q4_K_M 量化。**前提是对的**：调研核实 7600M XT 是 288 GB/s，F16 下 0.9B 每 token 要搬 ≈1.8 GB，288÷1.8 ≈ 160 tok/s 理论上限、乘经验效率 ≈91，而实测 96 tok/s——**它撞在显存带宽上**，所以减少每 token 的搬运量才是真杠杆，这也解释了为什么升级 Ollama 只有 −5%。
>
> Ollama 官方库只有 `latest`(F16) / `bf16` / `q8_0` 三个 tag，**没有 Q4_K_M**；社区包里挑了 `mradermacher/GLM-OCR-GGUF`（是少数几个**同时带 mmproj** 的，`GLM-OCR-i1-GGUF` 的 imatrix 版没有 mmproj，拉下来是个看不见图的模型）。
>
> | | F16 | **q8_0** | Q4_K_M |
> |---|---|---|---|
> | 大小 | 2.22 GB | 1.59 GB | 1.03 GB |
> | 速度（9 页干净均值） | 17.1 s/页 | **13.0（−24%）** | 10.8（−37%） |
> | 公式 exact/minor/degraded/**wrong** | 31/4/7/**3** | 31/4/7/**3** | 31/4/7/**5** |
> | 表格 | 0 行 | 0 行 | 0 行 |
>
> **结论：换 `glm-ocr:q8_0`。** 它的错误集合跟 F16 **逐条相同**（同样的行、同样的 ratio），而快 24%——70 页的论文从 20 分钟到 15 分钟。Q4 再快 17%，但多两条错，且**两条是同一类**：把散文包进 `$$`（`$$\text{Throughout the present article we denote by } |S| \text{ …}$$`），五个真公式反而跟 q8_0 逐字相同。它不是丢公式，是分不清"句子里出现了符号"和"这是一个独立公式"。加上那个包 `quantization_level` 报 `unknown`、缺 `tools` 能力、大概率没带 `RENDERER`/`PARSER`——**为了 2 秒不值得**。
>
> **两个假象都是我自己测出来的，记下来因为都会再犯：**
>
> **假象一：升级没用（−5%）。** 复测时我把两个复读页排除了，理由是"复读页的耗时由模型啰嗦多久决定，不由跑多快决定"——听起来站得住，实际上**升级的收益几乎全在那两页上**（181 秒 → 8.1 秒）。取样把要测的东西筛掉了。
>
> **假象二：Q4 慢 76%。** 跑 Q4 时 `/api/ps` 里还挂着上一个模型没过期（2.13 + 2.31 = 4.44 GB / 8 GB），全是显存争用。清空后单独跑是 **−37%**，与带宽理论同向。**跑基准前先 `keep_alive: 0` 卸干净**，现在 `confounds.py` 每次测量前都这么做。
>
> **顺带核实的显存问题。** 用户问起"AMD 8G 读成 4G"那个 bug：**存在，但是 APU/核显专属**（统一内存 carve-out 与 GTT 被混淆，issue #13677/#13173/#12342 全是 gfx1151/780M 这类）。这台是独显 RX 7600M XT，日志 `total="8.0 GiB" available="7.8 GiB"`，减去 457 MiB 的 `minimum` 预留得 7.4，三个数自洽，**没有腰斩**。另外那条 `amd.go:485 "AMD driver is too old"` 是**误报**——源码里它根本不解析版本，只是 `LookPath("amdhip64_7.dll")` 找不到就报，而 Ollama 用的是自带的 ROCm 7.1。
>
> **A9（丢表格）三个版本全都救不了**，量化与它无关。
>
> 2026-08-23 实测 rung 5（glm-ocr），以及一个反过来的发现：rung 1 并非无条件更好。用户问「你测测 glm-ocr 的效果」，并让另派 agent 查当下 SOTA。
>
> **样本与判据。** 3 篇 arXiv 论文 × 各 3 页数学最密的页（按 PyMuPDF 的数学字体字符数排序挑的，不是随手翻的）：`2503.03827`（现代、双栏、带大彩表）、`2410.11942`（现代、70 页、大量矩阵）、`cond-mat/9512169`（1995 年，legacy id，测字体年代）。**基准真值用 LaTeXML 的 `alttext`**——那不是对 PDF 的又一次识别，是作者自己写下的 TeX，所以对不上就是 OCR 错了，而不是两个猜测互相打架。调用参数逐字复刻管线：pdftoppm 150 DPI、RGB、LANCZOS 缩到 ≤2MP、JPEG q95、`/api/generate`、`num_ctx 16384`、`temperature 0`。
>
> **第一版评分器给出的数字是假的，必须说清楚。** 它拿整块 `$$…$$` 去比，于是把 `c_{x,\sigma}^{\dagger}` 和 `c^{\dagger}_{x,\sigma}` 判成不同、把作者用 `\begin{split}` 包起来的两行判成一行错、把 `t'` 和 `t^{\prime}` 判成不同——**Hubbard 那篇 12 个公式全对，它给了 0 个 exact**。分组和内容是两个问题，只有一个是 OCR 的责任。改成按公式**行**比、并把无内容差异的拼写归一（上下标顺序、`\rm`/`\text`、`\prime`、`\tag`）之后数字才可信。**这一段留在这里是因为下次还会犯**：自动匹配器只配当筛子，判决得回到页面图像上看。
>
> **结论：公式好，表格全丢，会复读。**
>
> | 维度 | 结果 |
> |---|---|
> | **公式** | 干净页 39 行：26 行逐字对上，另有约 10 行只是拼写变体（同一个数学），**真错 3 行** |
> | **散文** | 无表格页的文本回收率 **0.96–1.02**，基本一字不落 |
> | **表格** | **151 行进，0 行出**（A9） |
> | **速度** | 正常页 **17.1 秒/页**；复读页 181 秒 |
> | **复读** | 9 页中 **2 页**（A10） |
>
> 三处真错值得单看，因为它们**都不是乱码，而是像模像样的错**：`m-type anyons` 读成 `e-type anyons`（这两个是不同的任意子，而同一页上两个公式确实一个 m 一个 e，它把标签和主体接错了）；`\mathbb{Z}_{p_1^{k_1}}` 读成 `\mathbb{Z}_{p_1}^{k_1}`（CRT 分解，嵌套错了就是另一个群）；系数向量 (35) 少数了一个 `0`，于是 `-2` 落到了错的单项式底下。**这正是最贵的一类错**——它不会在校对时跳出来。
>
> **反过来的发现：rung 1 也会塌，而且是同一个东西塌的。** 查一条被判"错"的公式时发现，基准真值本身是 **1,900 字符的 `\mathchoice{\hbox{\hskip 2.98613pt…\resizebox{21328392}{273069}…`**——作者用了 LaTeXML 展不开的自定义宏（一个"宽波浪线盖在符号上"），于是 `alttext` 里装的是排版原语而不是数学。**那一条上 glm-ocr 的输出比 arXiv 自己的渲染更可用。** 统计三篇：`2410.11942` **10.2%**（211/2067）的公式带排版原语，最长一条 21,818 字符；`2503.03827` 1.1%；1995 那篇 **0.0%**。所以"rung 1 的公式是原样搬运的 TeX"这句话有前提——**宏展得开才成立**，展不开时它退化得比下面几级更难看。这不推翻阶梯顺序（10% 对 90%），但把 rung 1 写成无条件正确是错的。
>
> **慢在哪儿，量过了。** 拆开一页：图像编码 + prefill（2520 token）**0.9 秒 = 9%**，token 生成 **9.1 秒 = 91%**。把图从 2MP 缩到 0.5MP，10.0 秒只变成 9.3 秒，还掉字——**所以压图没用，瓶颈是生成吞吐**。模型确实整个在显存里（`/api/ps`: size_vram = 2.85 GB = 全部），不是掉回 CPU 了。
>
> 并行派出的调研 agent 给了同向的结论：`glm-ocr` = zai-org GLM-OCR 0.9B（MIT），**是当下的，没有更新版**，OmniDocBench v1.5 总分 94.62 排第一、公式 CDM 93.90。而 **Ollama 0.32.5 内置的 llama.cpp 停在 b7437（2025-12）**，缺两个 2026 年的 AMD/Vulkan 提速补丁（llama.cpp #19625 wave32 flash attention on AMD、#20551 use graphics queue on AMD）；有 AMD 机器上 Ollama ~34 tok/s vs 独立 llama.cpp ~52–56 tok/s 的对比报告。**这正好打在测出来的 91% 上**，所以第一步该换的是运行时不是模型——权重不动，质量不变。（未验证：没找到这块具体显卡上 GLM-OCR 走 LM Studio/Vulkan 的实测报告。）
>
> 换模型这条查下来是死路多于活路：PaddleOCR-VL-1.6 公式 9.65 分（比 MinerU 云端的 9.17 还高）且有官方 GGUF，但 **Windows + AMD 上确认零输出**，issue 被 maintainer 标 not planned；olmOCR 要 vLLM，ROCm 只面向 Linux 数据中心卡；Marker/Surya 在 AMD 上卡在 CUDA 依赖。唯一绕开 GPU 的是 Docling（IBM，MIT，CPU 中位 0.79 秒/页），但它自己的讨论区有一条标题就叫「Handling poor formula extraction performance」。
>
> **没测的还有一样，而且是最要紧的一样：扫描件。** 这 9 页全是原生电子版。rung 5 存在的理由恰恰是那些没有文本层的扫描件——A4 修完之后原生电子版更可能被 textlayer 这一级接走，真正掉到 rung 5 的会更偏向扫描件。这条留在 E 栏。
>
...
> 最后整理：2026-08-23（v1.12.4 之后；A12/B1/B2/D2 来自 DeepSeek 实测那一轮）。**都有复现证据**，不是猜测；每条注明证据、位置和建议改法。
> 修掉一条就从这里删掉，并在当天的 dated entry 里写清楚。
> **A1–A10 已全部处理完**（A1–A8 于 2026-08-22，A9/A10 于 2026-08-23 / v1.12.3）。编号只增不重排，旧讨论里引用的编号要一直有效。

### A. 已确认的 bug

**空的。** A1–A13 全部处理完（A12/A13 见 2026-08-23（日）条目）。

这一栏是空的，不代表没有 bug，只代表**没有已知且已复现的 bug**。下一条进来时按原格式加，
编号继续往下走（A12 起），不要重排——旧讨论里引用的编号得一直有效。

### B. 质量问题（不是 bug，是选择）

| # | 问题 | 证据 | 建议 |
|---|---|---|---|
| ~~B1~~ | ~~**`pymupdf4llm` 的 `write_images=True` 导出的是"每个嵌入图像对象"，不是"图"**~~ **已修 2026-08-23（日）** | 复测确认并且更难看：一篇 23 页、**只有 4 张图**的论文吐出 **117 个文件**，其中 102 张小于 200×200，最小的 301×24 和 40×24——**是把单行行间公式渲染成了图片**，而且每一张都被内联进 markdown。`image_size_limit` 没用（0.05 与 0.25 输出逐字节相同，见下） | 已改为默认关闭（`ingest.textlayer_images`，默认 `false`），并**明说**跳过了图：finding `figures-not-exported` 指向会裁图的 OCR 路线 |
| ~~B2~~ | ~~**`pymupdf4llm` 把表格塌成一行**~~ **复现不了，条件比原记录窄得多** | 2026-08-23（日）在 pymupdf4llm **1.28.2**（记录 B2 时装的就是这个版本，不是版本升级修好的）上四种输入全部正常出 markdown 表格：全框线表、booktabs、完全无框线、以及**一篇真实 23 页论文的全部 10 张表**（census 205 行 → markdown 285 个 pipe 行）。原始证据里 `\hline` 漏成 `height`，指向某一类特定文档，而那份文档已经找不到了 | 不删条目、改为**钉住当前的正确行为**（`test_batch.py` 三个测试）。真回归了由测试说话，而不是由一条没人复测的笔记说话。另：`tables-dropped` 门本来就覆盖"表全没了"，也一并钉了 |
...
| D2 | **配置有两套表示，需要代码同步** —— **漂移已堵（2026-08-23 日），两份表示仍在** | 预言当天兑现：`magi setup --full` 直接给 `profile` 赋值，而 `feature_enabled` 只允许 `profile: kb-only` **否决**，于是新键一旦是 False，翻 profile 根本翻不动——命令打印"profile set to full"，任务追踪纹丝不动。现在三个写入点全部收敛到 `set_feature`（它同时写两处），并加了一条**结构性**测试：`features.py` 之外任何文件出现 `["profile"] =` 就失败。这条测试当场又抓出第二个写入点 | 两份表示为了向后兼容仍然保留。**要真正合并，得让 `profile` 变成派生值而不是存储值**，那需要迁移，成本不在"顺手"这一档 |
...
| ~~D7~~ | ~~**秘密被迫存 N 份，而且有一条看起来能用、实际永远轮不到的全局路径**~~ **已修 v1.12.4** | `models.ocr` 之类的设置每个工作区一份还说得过去，`ocr.mineru_api_token` 不行——WebUI 的配置编辑器写的是 `ws / "config.yaml"`（`api.py`），而 `find_config_yaml` 是**首个命中即返回**：向上走撞到第一个工作区/hub 根下的 config.yaml 就用它，只有一路走到头才回退 `~/.config/magi/config.yaml`。**每个 `magi init` 出来的工作区根下都有 config.yaml**，所以那条全局路径永远被挡住。实测：用户机器上同一个 JWT 存了三份，而 `~/.config/magi/config.yaml` 根本不存在。**这比没有全局层更糟**——代码里有这条回退，看起来像是能用的 |
...
- ~~本地 OCR（rung 5）在数学上的质量从未测过~~ **已测，2026-08-23，见当天条目**：公式好（干净页 39 行里 3 行真错），表格全丢（A9），会复读（A10）。**仍没测的是扫描件**——9 页样本全是原生电子版 PDF，而 rung 5 存在的理由正是扫描件
- DocMind 在扫描件上的表现（见 C）
- **DeepSeek 视觉路线（见 2026-08-23 日条目）没测的三件事**：① **扫描件**——九页全是原生电子版，而这正是视觉路线存在的理由；
  ② **训练集污染**——三篇都是 arXiv 论文，模型可能见过。反证是整页模式在 toric p5 上 22 行一行没对、切图后 22/22，
  **改善跟着像素预算走、不跟着论文身份走**；但严格排除需要一份不可能在训练集里的文档；
  ③ 非物理学科；切图粒度只试了 2×2（没试 1×3 / 3×3），重叠区去重的可靠性也没单独验证
- **真实使用者规模没测出来，而且现有数据框不出区间。** 无歧义是人的只有 7 star / 1 fork / 20 唯一访客；
  另一端是 ~435 次带 pip UA 的真实安装、每天约 30 个非 CI 的唯一 cloner。两端差一个量级，
  中间被自己的动作污染了——一周发了 8 个版本，每次都在给自动化发信号。
  **需要的是一个"什么都不做"的窗口**：停发一周再看每天的真实安装数还在不在 70–80。
  在不在，答案完全不同，而这个实验不花钱只花耐心（见 2026-08-23（六）条目的四条判读法）
