...
>
> 最后更新：2026-08-23 · 当前阶段：**v1.12.4 已发布**，之后有未发布改动（A12/A13、B1、rung 5 表格与切图）。此前：M0–M9 全部完成。
>
...
- ~~本地 OCR（rung 5）在数学上的质量从未测过~~ **已测，2026-08-23，见当天条目**：公式好（干净页 39 行里 3 行真错），~~表格全丢（A9）~~**表格是提示词丢的，不是模型丢的，已修（见 8-23 日·续）**，会复读（A10，仍在）。**仍没测的是扫描件**——9 页样本全是原生电子版 PDF，而 rung 5 存在的理由正是扫描件
- DocMind 在扫描件上的表现（见 C）
- **切图对扫描件有没有用，没测。** 判据是 PyMuPDF 在文本层里找到表格，而扫描件没有文本层，
  所以扫描件一律整页读——和改动之前一样，不会更差，但也拿不到这次的收益。
  真要覆盖，需要一个不依赖文本层的表格探测（版面模型或投影分析），那是另一件事
