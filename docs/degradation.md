...
| **A host's model list** (`agy models`) | The model picker in the config panel | Four rungs: the record's own `models:` wins, then a cached list under 24h old, then the CLI is asked (20s), then nothing | `source` names the rung that answered — `static`, `cache`, `live` or `none` — so a stale list reads as stale, and with `none` the panel takes a typed model id instead of pretending to know | `core/hosts.py` (`models`, `parse_models`, `MODELS_TTL`, `LIST_TIMEOUT`) |
...
| **The reviewer that was picked** (a vendor's quota, a timeout, a crash) | `magi review`, and the browser's review button | The next installed CLI is asked, cross-vendor first and the author's own last; a host that failed once in a batch is not asked about the next claim | The verdict's signature line says *"X was asked first and failed (why)"*; with no host left, *"no reviewer answered"* and the claim stays unreviewed | `review.py` (`reviewers`, `review`) |
| **The call switch** | Every model call MAGI makes on its own initiative | There is no weekly budget (cancelled 2026-09-03; `core/ledger.py` says why). Calls are counted, never refused, except by the master switch | `research.llm_calls: false` refuses with *"switched off … nothing counts as reviewed"*; `MAP.md` and the dashboard show the week's count, kinds, tiers and failures | `core/ledger.py` (`check`, `summary`) |
