...
## When to use
Any question about what this project knows. Also when you are about to answer
one from memory.
...
3. About one concept: `magi wiki context --name "<concept>"` gives every paragraph that
...
4. Structural questions — what links to what, which sources back a claim,
   ...
   same without SQL. Exact strings: `magi grep "<text>" wiki/`.
5. Answer with inline `[[wikilinks]]` to the cards you actually read. Where
   the project does not know, say that instead of filling the gap.
6. A question the project cannot answer but should be able to is a gap worth
