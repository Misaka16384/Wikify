...
2. Read the workspace before judging: `magi stats wiki-summary` and the open
...
4. `magi search "<title>"` before keeping one: a paper the library already
   covers is not new information, it is a duplicate.
5. `magi radar triage --decision accept` / `--decision dismiss` per candidate.
   Never hand-edit the digest's frontmatter to record a decision — the triage
   ledger is what the next run reads.
...
- A citation-gap report asks a different question: whether this library *owes*
