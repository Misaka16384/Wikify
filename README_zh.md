...
- **智能自愈 (Auto-Healing)**：智能 Linter 能够在合并重构期间自动检测死链，并借助全局别名路由系统（Alias Routing）全自动自愈知识图谱中的失效链接。它还搭载了强大的 YAML 级自愈机制，能自动扫平并修复大模型“幻觉”导致的元数据语法崩溃（如未转义的 LaTeX 反斜杠）。
...
- **`wiki_semantic_link`**: 这是一个静默运行的技能。它会对所有概念进行向量扫描，并在卡片底部自动注入美观的 `[[相关链接|别名]]`。搭载了极速 MD5 增量缓存机制实现未更改概念的 API 零开销，并内置了智能的占位符清洗策略，彻底杜绝“空概念”由于模板废话而引发的假阳性误报聚类。
